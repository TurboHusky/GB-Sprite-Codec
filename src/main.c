#include <stdint.h>
#include <stddef.h>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>

enum rle_data_t
{
    RUN = 0,
    DATA = 1
};

struct bit_buffer_t
{
    uint8_t *data;
    size_t size;
    size_t byte_index;
    int8_t bit_index;
};

struct sprite_t
{
    uint8_t width;
    uint8_t height;
    uint8_t *data;
};

// Buffer for bitplane using 8x8px tiles @ 1bpp
#define PX_PER_BYTE 8
#define BUFFER_WIDTH_IN_TILES 7
#define BUFFER_HEIGHT_IN_TILES 7
#define TILE_WIDTH 1
#define TILE_HEIGHT 8
#define BUFFER_SIZE (BUFFER_WIDTH_IN_TILES * TILE_WIDTH * BUFFER_HEIGHT_IN_TILES * TILE_HEIGHT)

void draw_bitplane(const uint8_t *const data, const uint8_t width, const uint8_t height, const char *const filename)
{
    FILE *fp = fopen(filename, "wb");
    fprintf(fp, "P6\n%d %d\n255\n", width * TILE_WIDTH * 8, height * TILE_HEIGHT);
    uint8_t bw[] = {0x00, 0x00, 0x00, 0xff, 0xff, 0xff};

    for (int y = 0; y < height * TILE_HEIGHT; y++)
    {
        for (int x = 0; x < width; x++)
        {
            for (int shift = 7; shift >= 0; shift--)
            {
                if ((data[x * (height * TILE_HEIGHT) + y] >> shift) & 0x01)
                {
                    fwrite(bw, sizeof(uint8_t), 3, fp);
                }
                else
                {
                    fwrite(bw + 3, sizeof(uint8_t), 3, fp);
                }
            }
        }
    }

    fclose(fp);
}

void decode(struct bit_buffer_t *bbuf, struct sprite_t *sprite)
{
    uint16_t bitplane_size = sprite->width * TILE_WIDTH * sprite->height * TILE_HEIGHT * PX_PER_BYTE;
    uint16_t bitplane_count = 0;

    enum rle_data_t packet_type = (bbuf->data[bbuf->byte_index] >> bbuf->bit_index) & 0x01;
    bbuf->bit_index--;

    uint8_t x = 0;
    uint8_t y = 0;
    int8_t shift = 6;

    memset(sprite->data, 0, bitplane_size >> 3);

    while (bitplane_count < bitplane_size)
    {
        if (packet_type == RUN)
        {
            uint64_t run_length = 0;
            uint64_t value = 0;
            uint8_t bit_count = 0;

            do
            {
                if (bbuf->bit_index == -1)
                {
                    bbuf->byte_index++;
                    if (bbuf->byte_index >= bbuf->size)
                    {
                        printf("ERROR: RLE read out of bounds");
                        return;
                    }
                    bbuf->bit_index = 7;
                }

                run_length <<= 1;
                run_length |= (bbuf->data[bbuf->byte_index] >> bbuf->bit_index) & 0x01;
                bbuf->bit_index--;
                bit_count++;
            } while (run_length & 0x0000000000000001);

            for (size_t i = 0; i < bit_count; i++)
            {
                if (bbuf->bit_index == -1)
                {
                    bbuf->byte_index++;
                    if (bbuf->byte_index >= bbuf->size)
                    {
                        printf("ERROR: RLE read out of bounds");
                        return;
                    }
                    bbuf->bit_index = 7;
                }

                value <<= 1;
                value |= (bbuf->data[bbuf->byte_index] >> bbuf->bit_index) & 0x01;

                bbuf->bit_index--;
            }

            uint64_t N = run_length + value + 1;
            bitplane_count += N << 1;
            // printf("R-%lu ", N);

            uint64_t delta_x = (y + N) / (BUFFER_HEIGHT_IN_TILES * TILE_HEIGHT);
            y = (y + N) % (BUFFER_HEIGHT_IN_TILES * TILE_HEIGHT);
            x += (delta_x - (shift >> 1) + 3) >> 2;
            shift = (shift - (delta_x << 1)) % 8;
            // printf("%u %u %d\n", x, y, shift);

            packet_type = DATA;
        }
        else
        {
            if (bbuf->bit_index < 1)
            {
                bbuf->byte_index++;
                if (bbuf->byte_index >= bbuf->size)
                {
                    printf("ERROR: RLE read out of bounds");
                    return;
                }
                bbuf->bit_index += 8;
            }

            uint8_t bit_pair = (bbuf->bit_index == 8) ? ((bbuf->data[bbuf->byte_index - 1] << 1) & 0x02) | (bbuf->data[bbuf->byte_index] >> 7) : (bbuf->data[bbuf->byte_index] >> (bbuf->bit_index - 1)) & 0x03;

            if (bit_pair)
            {
                sprite->data[x * BUFFER_HEIGHT_IN_TILES * TILE_HEIGHT + y] |= (bit_pair << shift);
                // printf("[%u - %u, %u, %u] ", bit_pair, x, y, shift );

                y++;
                if (y >= BUFFER_HEIGHT_IN_TILES * TILE_HEIGHT)
                {
                    y = 0;
                    shift -= 2;
                    if (shift > 0)
                    {
                        shift = 6;
                        x++;
                    }
                }

                bitplane_count += 2;
            }
            else
            {
                packet_type = RUN;
            }
            bbuf->bit_index -= 2;
        }
    }

    if (bbuf->bit_index < 0)
    {
        bbuf->byte_index++;
        if (bbuf->byte_index >= bbuf->size)
        {
            printf("ERROR: RLE read out of bounds");
            return;
        }
        bbuf->bit_index += 8;
    }
}

int main(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    // uint8_t test_data[] = {};
    uint8_t input[] = {};

    uint8_t *buffer = malloc(BUFFER_SIZE);

    struct bit_buffer_t bit_ptr = {
        .data = input,
        .size = sizeof(input),
        .byte_index = 1,
        .bit_index = 6};

    struct sprite_t v_sprite = {
        .data = buffer};
    v_sprite.width = input[0] >> 4;
    v_sprite.height = input[0] & 0x0f;
    uint8_t primary_buffer = input[1] >> 7;

    printf("Decoding %ux%u, primary buffer: %u\n", v_sprite.width, v_sprite.height, primary_buffer);

    decode(&bit_ptr, &v_sprite);
    draw_bitplane(buffer, v_sprite.width, v_sprite.height, "buffer1.ppm");
    printf("Encoding bit: %02x  %lu %d\n", bit_ptr.data[bit_ptr.byte_index], bit_ptr.byte_index, bit_ptr.bit_index);

    uint8_t encoding_method = (bit_ptr.data[bit_ptr.byte_index] >> bit_ptr.bit_index) & 0x01;
    bit_ptr.bit_index--;
    if (bit_ptr.bit_index < 0)
    {
        bit_ptr.byte_index++;
        if (bit_ptr.byte_index >= bit_ptr.size)
        {
            printf("ERROR: RLE read out of bounds");
            return 1;
        }
        bit_ptr.bit_index += 8;
    }

    if (encoding_method != 0)
    {
        encoding_method = (encoding_method << 1) | ((bit_ptr.data[bit_ptr.byte_index] >> bit_ptr.bit_index) & 0x01);
        bit_ptr.bit_index--;
        if (bit_ptr.bit_index < 0)
        {
            bit_ptr.byte_index++;
            if (bit_ptr.byte_index >= bit_ptr.size)
            {
                printf("ERROR: RLE read out of bounds");
                return 1;
            }
            bit_ptr.bit_index += 8;
        }
    }
    printf("Encoding: %u\n", encoding_method);

    printf("Input: %02x  %lu %d\n", bit_ptr.data[bit_ptr.byte_index], bit_ptr.byte_index, bit_ptr.bit_index);
    decode(&bit_ptr, &v_sprite);
    draw_bitplane(buffer, v_sprite.width, v_sprite.height, "buffer2.ppm");

    free(buffer);
}